...
const defaultRoutes = [
  ...
  {
    url: "/admin/configuracoes/servicos",
    name: "Serviços",
  },
  {
      ...
      <div className="flex text-black overflow-x-auto border-b border-gray-100">
        {defaultRoutes.map((route) => (
          <LinkButton
            key={route.url}
            href={route.url}
            className={`px-6 py-1 border-b-2 border-gray-200 hover:border-gray-400 hover:text-black hover:border-blue-900 transition-all duration-300 ${
              pathname === route.url ? " border-blue-500 bg-blue-100" : ""
            }`}
          >
            {route.name}
          </LinkButton>
